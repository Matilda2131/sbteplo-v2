...
SYS = "Ты Мимо, помощник Саши. На ты, с эмодзи, 2-3 предложения. Помогаешь с сайтом sbteplo.ru, SEO, кодом."
...
def seo_report():
    try:
        ...
        return "Сайт: %s, %dKB" % (r.status_code, len(r.text)//1024)
    except: return "Ошибка проверки"

def handle(cid, text):
    try:
        if text == "/start":
            send(cid, "Привет! Я Мимо. Команды: /seo /list. Или просто пиши!")
            return
        if text == "/seo" or text == "/report":
